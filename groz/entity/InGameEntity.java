...
public class InGameEntity {
	...
	public void damage(InGameEntity attacker, Attack attack, Random rand,
			boolean isCrt) {
		int EAttack = (int) attacker.preformEffect(EnumStatType.ATK,
				attacker.getAttack() - rand.nextInt(attacker.getAttack() / 2));
		int Defense = (int) this.preformEffect(EnumStatType.DEF, this.DEF
				- rand.nextInt(this.DEF / 2));
	...
	public void heal(Attack attack, Random rand) {
		int heal = (int) this.preformEffect(EnumStatType.ATK,
				rand.nextInt(this.ATK / 2));
