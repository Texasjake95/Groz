...
public class Monster extends Entity {
	...
	public static final Monster bat = new Monster("Bat", new DualStats(new Stats(0, 15, 3, 2, 7, 4, 1)), 0, Speed);
	public static final Monster zombie = new Monster("Zombie", new DualStats(new Stats(0, 13, 5, 2, 7, 4, 1)), 1, Attack);
	public static final Monster wolf = new Monster("Wolf", new DualStats(new Stats(0, 12, 3, 2, 5, 4, 1)), 2, Speed);
	public static final Monster golem = new Monster("Golem", new DualStats(new Stats(0, 20, 2, 6, 1, 1, 1)), 3, Defense);
	public static final Monster rat = new Monster("Rat", new DualStats(new Stats(0, 8, 2, 3, 3, 2, 1)), 4, Agility);
	public static final Monster fairy = new Monster("Fairy", new DualStats(new Stats(0, 12, 3, 3, 3, 4, 1)), 5, Agility);
	public static final Monster dwarf = new Monster("Dwarf", new DualStats(new Stats(0, 13, 3, 4, 1, 2, 1)), 6, Tank);
	...
	public Monster(String name, DualStats stats, int id, EnumMonsterType type, boolean isCopy)
	{
		super(name, stats);
		if (!isCopy)
		{
	...
	public Monster copy()
	{
		return new Monster(this.getName(), this.getStats(), this.ID, this.type, true);
	}
	...
	public void setLvl(Player player, ZoneBase zone)
	{
		if (player.getStats().getLvl() >= zone.getRecommendedLvl())
		{
			if (player.getStats().getLvl() > 2)
			{
				...
				{
					this.getStats().setLvl(player.getStats().getLvl() - rand.nextInt(2));
				}
				else
				{
					this.getStats().setLvl(player.getStats().getLvl() + rand.nextInt(2));
			...
			{
				this.getStats().setLvl(rand.nextInt(player.getStats().getLvl()));
			}
		}
		else
		{
			if (rand.nextDouble() < .50d)
			{
				this.getStats().setLvl(zone.getRecommendedLvl() - rand.nextInt(2));
			}
			else
			{
				this.getStats().setLvl(zone.getRecommendedLvl() + rand.nextInt(2));
	...
	{
		this.getStats().setMaxHealth(this.getStats().getBaseHealth() + this.getStats().getHP() * 5);
